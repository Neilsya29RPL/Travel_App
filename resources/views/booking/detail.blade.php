...
<a href="{{ route('bookings.index') }}" class="btn secondary" style="margin-bottom:14px">Kembali ke Daftar</a>
    ...
    <div class="card" style="padding:16px; margin-bottom:16px">
        <div style="display:flex; gap:24px; align-items:flex-start; flex-wrap:wrap">
            ...
            <div style="min-width:260px">
                ...
                <p><strong>Jumlah Tamu:</strong> {{ $booking->num_guests ?? '-' }}</p>
                <p><strong>Durasi (hari):</strong> {{ $booking->duration_days ?? '-' }}</p>
                <p><strong>Total:</strong> Rp {{ number_format(($booking->total_amount ?? 0) * 1000, 0, ',', '.') }}</p>
            </div>
        </div>
    </div>
        ...
        <table class="table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{ $p->method ?? '-' }}</td>
                    <td>Rp {{ number_format(($p->amount ?? 0) * 1000, 0, ',', '.') }}</td>
                    <td>{{ $p->status ?? '-' }}</td>
