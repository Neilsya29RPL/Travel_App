...
@if (session('error'))
    <div class="alert alert-danger"><div class="icon">!</div><div>{{ session('error') }}</div></div>
@endif
...
@else
        ...
        <form method="POST" action="{{ route('booking.create') }}" class="grid">
            ...
            <select name="destination_index" required>
                @foreach (($rec['destinations'] ?? []) as $idx => $d)
                    <option value="{{ $idx }}" {{ (isset($defaultDestIndex) && $defaultDestIndex == $idx) ? 'selected' : '' }}>{{ $d['name'] }} — Rp {{ number_format(($d['est_budget'] ?? 0), 0, ',', '.') }} / hari</option>
                @endforeach
            </select>
            ...
            <select name="accommodation_index" required>
                @foreach (($rec['accommodations'] ?? []) as $idx => $a)
                    <option value="{{ $idx }}" {{ (isset($defaultAccIndex) && $defaultAccIndex == $idx) ? 'selected' : '' }}>{{ $a['name'] }} ({{ $a['type'] }}) — Rp {{ number_format(($a['price_per_night'] ?? 0), 0, ',', '.') }}/malam</option>
                @endforeach
            </select>
    ...
    @if ($lastBooking)
        ...
        <p>Total: Rp {{ number_format(($lastBooking->total_amount ?? 0), 0, ',', '.') }}</p>
