                    ...
                    <div class="cards" id="accCards">
                        @foreach ($accommodations as $acc)
                            <div class="card-item selectable" data-index="{{ $acc['rec_index'] ?? $loop->index }}" data-total="{{ (int)($acc['total_estimate'] ?? 0) }}">
                                ...
                                <div class="body">
                                    <div class="badge">{{ $acc['type'] ?? 'Tipe' }} · {{ $acc['price_per_night'] ? ('Rp '.number_format(($acc['price_per_night'] ?? 0)*1000,0,',','.').'/malam') : '—' }}</div>
                                    <h4 style="margin:8px 0 6px">{{ $acc['name'] ?? 'Akomodasi' }}</h4>
                                    <div style="color:#cbd5e1; font-size:13px;">Durasi: {{ $nights }} malam</div>
                                    <div style="margin-top:8px"><strong>Rp {{ number_format(($acc['total_estimate'] ?? 0)*1000,0,',','.') }}</strong> estimasi</div>
                                </div>
                            </div>
                        @endforeach
                    </div>
                    <div class="bottom-actions">
                        <div>
                            ...
                            @if(!is_null($prefBudgetMax))
                                <div class="budget-line">Budget maksimum: Rp {{ number_format(($prefBudgetMax ?? 0)*1000,0,',','.') }} @if(!is_null($prefBudgetMin)) · Min: Rp {{ number_format(($prefBudgetMin ?? 0)*1000,0,',','.') }} @endif</div>
                            @endif
                        </div>
                        <div style="display:flex; gap:8px">
                            <a href="{{ route('budget.index') }}" id="budgetLink" class="btn btn-secondary">Cek Estimasi Anggaran</a>
                            <a href="{{ route('booking.index') }}" class="btn btn-primary">Booking</a>
                        </div>
                    </div>
                    <script>
                        (function(){
                            ...
                            const fmt = (n)=> new Intl.NumberFormat('id-ID').format(n*1000);
                            const destIndex = (function(){
                                try { return {{ !empty($selectedIndexes) ? (int) $selectedIndexes[0] : 0 }}; } catch(e){ return 0; }
                            })();
